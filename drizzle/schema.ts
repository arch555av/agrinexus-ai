...
export const nudges = mysqlTable("nudges", {
  id: varchar("id", { length: 40 }).primaryKey(), userId: int("userId").notNull(), farmId: varchar("farmId", { length: 40 }), title: varchar("title", { length: 160 }).notNull(), message: text("message").notNull(), priority: mysqlEnum("priority", ["low", "medium", "high"]).notNull(), status: mysqlEnum("status", ["pending", "completed", "dismissed"]).default("pending").notNull(), readAt: timestamp("readAt"), dueAt: timestamp("dueAt"), completedAt: timestamp("completedAt"), createdAt: timestamp("createdAt").defaultNow().notNull(),
});
